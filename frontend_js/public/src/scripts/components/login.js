document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("btn-login").addEventListener("click", (ev) => {
    ...
    const body = JSON.stringify({ username: uname.value, password: pw.value });
    const method = "POST";
    ...
    fetch(url, { method, body, headers })
      ...
      .then(({ status, json }) => {
        if (status === 404) {
          e.innerText = "Username tidak ditemukan";
        } else if (status === 401) {
          e.innerText = "Password salah";
        } else if (status === 200) {
          e.innerText = "Login berhasil, mohon ditunggu";
          localStorage.setItem("chem-token", json.token);
          window.location.href = "/src/pages/mainMenu.html";
        } else if (json.error) {
